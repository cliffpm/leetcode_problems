...
| 347 | [Top K Frequent Elements](https://leetcode.com/problems/top-k-frequent-elements/description/)           |          |      |       | Medium     |     | Multiple ways. Bucket sort is fastest because its O(N), using hashmap as frequency counter and Max-Heap in place is O(N+klog(N)), because N for the heapify and hashmap creation, klog(N) for every root pop in heap because sink / swim.    |
